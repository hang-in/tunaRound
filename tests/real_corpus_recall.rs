    ...
    StoredSession {
        messages: vec![
            ...
            msg(
                1,
                "opus",
                "FTS5의 unicode61 토크나이저는 한국어를 형태소로 못 쪼개므로, Kiwi나 lindera로 미리 형태소 분해해 공백으로 조인한 텍스트를 색인한다.",
            ),
            msg(
                2,
                "opus",
                "형태소 토큰에 더해 raw 토큰까지 같이 색인해서 외래어 임베딩이 조사째 원형으로 남도록 한다.",
            ),
            msg(
                3,
                "opus",
                "임베딩 모델을 bge-m3에서 다른 것으로 바꿔도 content_hash가 같아 증분 가드가 오래된 벡터를 건너뛴다. 해시에 모델 id와 차원을 넣어야 한다.",
            ),
            msg(
                4,
                "opus",
                "검색이 전역이라 세션이나 시간 필터가 없어 옛 결정이 최신 결정처럼 프롬프트에 주입될 수 있다.",
            ),
            msg(
                5,
                "opus",
                "버려진 분기의 발언이 검색으로 끌려와 현재 분기 프롬프트에 섞일 수 있다.",
            ),
            msg(
                6,
                "opus",
                "FTS 키 리스트와 벡터 키 리스트를 reciprocal rank fusion으로 융합한다. k는 60이다.",
            ),
            msg(
                7,
                "opus",
                "recall at k와 MRR과 precision at k를 재는 회귀 게이트 벤치마크가 코드로 있어 검색 품질 저하를 막는다.",
            ),
            msg(
                8,
                "opus",
                "retrieved 주입은 개수만 제한하고 글자수 상한이 없어 긴 발언 다섯 개면 프롬프트가 팽창한다.",
            ),
            msg(
                9,
                "opus",
                "pull 모드 계측에서 프롬프트 길이가 전사 길이와 탈동조돼 claude가 9770자에서 429자로 줄었다.",
            ),
            msg(
                10,
                "opus",
                "KiwiWrapper의 unsafe impl Send는 C 포인터를 Mutex 직렬화에 의존하는 잠재 위험이다.",
            ),
            // 37b034cb(06-30 캐시 무효화):2
            msg(
                11,
                "opus",
                "임베딩과 형태소 토큰화는 콘텐츠 주소 방식이라 해시에 model id를 넣으면 무효화 자체가 필요 없다.",
            ),
            msg(
                12,
                "opus",
                "진짜 무효화가 필요한 건 파생된 쿼리 결과뿐이라 인덱스 세대 스탬프로 지연 무효화한다.",
            ),
            msg(
                13,
                "opus",
                "세대 입도를 세션이 아니라 인덱스 단위로 두면 무관한 세션의 캐시가 살아남는다.",
            ),
            // 6274470d:89
            msg(
                14,
                "opus",
                "단일 프로세스가 HTTP MCP 코어를 바인드하고 REPL을 동시에 구동하며 bearer 인증이 401과 200으로 동작한다.",
            ),
            // dff85fb8(07-01):334/352/159/186
            msg(
                15,
                "opus",
                "codex exec는 read-only 샌드박스를 유지한 채로는 MCP 도구 호출을 자동 승인할 방법이 없다.",
            ),
            msg(
                16,
                "opus",
                "codex는 규칙 준수가 강해서 read-only를 샌드박스가 아니라 프롬프트 지시로 강제할 수 있다.",
            ),
            msg(
                17,
                "opus",
                "recency 랭킹은 messages에 created_at 컬럼을 더하고 다른 세션의 오래된 후보만 소폭 강등한다.",
            ),
            msg(
                18,
                "opus",
                "debug_retrieve 디버그 출력에 created_at과 recency 강등 표시를 더해 눈으로 확인할 수 있게 했다.",
            ),
            // e5a848d3(06-30 설계토론: 리프레시 토큰 회전):8 - 다른 세션·다른 도메인(auth/보안)
            msg(
                19,
                "opus",
                "리프레시 토큰 회전이 막아주는 탈취 탐지는 리프레시 토큰이 브라우저 JS로 읽히는 localStorage에 장기 보관될 때만 의미가 있다.",
            ),
            msg(
                20,
                "opus",
                "access는 메모리 전용, refresh는 OS keychain, SPA는 httpOnly 쿠키로 두면 회전이 막는 공격면이 설계상 닫힌다.",
            ),
            msg(
                21,
                "opus",
                "opaque refresh에 jti denylist와 token_version을 두면 로그아웃이나 유출 의심 시 토큰 패밀리를 한 번에 폐기할 수 있다.",
            ),
            msg(
                22,
                "opus",
                "짧은 access TTL과 refresh의 절대 만료와 idle 만료를 함께 적용하면 회전 없이 위협모델을 덮는다.",
            ),
            msg(
                23,
                "opus",
                "리프레시 토큰이 브라우저 JS가 닿는 저장소로 가는 순간 reuse-detection과 함께 토큰 회전을 도입한다.",
            ),
        ],
        head: Some(23),
    }
...
fn real_corpus_fts_recall() {
    ...
    let ss = corpus();
    store
        .save_session("real", &ss, |t| tok.fts_index(t))
        .expect("save_session");
    ...
    for (q, gold, kind) in QUERIES {
        ...
        let (r3, r5) = (
            recall_at_k(&retrieved, gold, K3),
            recall_at_k(&retrieved, gold, K5),
        );
        let (p3, p5) = (
            precision_at_k(&retrieved, gold, K3),
            precision_at_k(&retrieved, gold, K5),
        );
    ...
    assert!(
        mr5 >= 0.88,
        "mean recall@5 회귀: {mr5:.3} < 0.88 (실코퍼스 baseline 0.944)"
    );
    assert!(
        mp5 >= 0.45,
        "mean precision@5 회귀: {mp5:.3} < 0.45 (실코퍼스 baseline 0.508)"
    );
}
...
fn real_corpus_hybrid_recall() {
    ...
    store_w
        ...
        .expect("index_vectors (터널?)");
    drop(store_w);

    let id_of =
        |c: &str| -> Option<u64> { ss.messages.iter().find(|x| x.content == c).map(|x| x.id) };
    ...
    println!(
        "{:<26} {:>7} {:>7} {:>7} {:>7}  종류",
        "질의", "ftsR", "ftsM", "hybR", "hybM"
    );
    println!("{:-<80}", "");
    for (q, gold, kind) in QUERIES {
        let fts_ids: Vec<u64> = store_fts
            .search(&tok.fts_query(q), K)
            .unwrap_or_default()
            .iter()
            .map(|h| h.msg_id)
            .collect();
        let hyb_ids: Vec<u64> = hybrid
            .retrieve(q, K)
            .unwrap()
            .iter()
