    ...
    StoredSession {
        messages: vec![
            msg(
                1,
                "claude",
                "JWT 기반 인증을 어떻게 설계할까. 액세스 토큰과 리프레시 토큰을 분리하자.",
            ),
            msg(
                2,
                "codex",
                "세션 방식보다 토큰 방식이 수평 확장에 유리하다.",
            ),
            msg(
                3,
                "claude",
                "검색은 형태소 FTS와 벡터 임베딩을 RRF로 융합한 하이브리드로 간다.",
            ),
            msg(
                4,
                "codex",
                "재주입을 줄이려면 최근 몇 턴만 넣고 나머지는 검색으로 끌어온다.",
            ),
            msg(
                5,
                "claude",
                "Redis 버스로 여러 프로세스가 한 세션을 공유하고 재개한다.",
            ),
            msg(
                6,
                "codex",
                "스냅샷과 이벤트 스트림으로 멀티세션을 미러링한다.",
            ),
        ],
        head: Some(6),
    }
    ...
    for (q, expect) in queries {
        ...
        println!(
            ...
            if lex.is_empty() {
                ...
                lex.iter()
                    .map(|h| format!("#{} {:.2} {}", h.msg_id, h.score, snip(&h.content)))
                    .collect::<Vec<_>>()
                    .join(" | ")
            }
        );
