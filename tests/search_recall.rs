    ...
    StoredSession {
        messages: vec![
            msg(
                1,
                "claude",
                "인증 방식을 정하자. 세션 쿠키 대신 토큰 기반으로 가는 게 확장에 유리하다.",
            ),
            msg(
                2,
                "codex",
                "동의한다. 액세스 토큰은 짧게, 리프레시 토큰으로 갱신하는 구조가 안전하다.",
            ),
            msg(
                3,
                "claude",
                "토큰을 어디 보관하지? 로컬스토리지는 XSS에 취약하니 httpOnly 쿠키를 쓰자.",
            ),
            msg(
                4,
                "codex",
                "로그인 흐름은 OAuth 위임으로 가면 비밀번호를 직접 안 다뤄도 된다.",
            ),
            msg(
                5,
                "claude",
                "검색은 형태소 분석으로 색인하고 FTS5 전문검색으로 질의하자.",
            ),
            msg(
                6,
                "codex",
                "외래어가 문제다. 임베딩 같은 단어가 형태소 분석에서 음절로 쪼개진다.",
            ),
            msg(
                7,
                "claude",
                "그러면 raw 토큰을 같이 색인해서 외래어를 원형으로 살리자.",
            ),
            msg(
                8,
                "codex",
                "의미 검색이 필요하면 벡터 임베딩을 붙여 하이브리드로 융합하면 된다.",
            ),
            msg(
                9,
                "claude",
                "맥락이 길어지면 매 턴 전체를 다시 넣는 건 토큰 낭비다.",
            ),
            msg(
                10,
                "codex",
                "최근 몇 턴만 유지하고 나머지는 검색으로 필요할 때 끌어오자.",
            ),
            msg(
                11,
                "claude",
                "요약을 만들어 라운드 사이에 이월하면 재전송을 더 줄일 수 있다.",
            ),
            msg(
                12,
                "codex",
                "여러 프로세스가 한 토론을 공유하려면 Redis로 상태를 미러링하자.",
            ),
            msg(
                13,
                "claude",
                "스냅샷과 이벤트 스트림을 함께 두면 끊겨도 재개가 된다.",
            ),
            msg(
                14,
                "codex",
                "참가자가 늘면 좌석마다 역할을 주입해서 발언을 시켜야 한다.",
            ),
            msg(
                15,
                "claude",
                "로컬 LLM도 좌석으로 받자. ollama나 lmstudio를 HTTP로 부르면 된다.",
            ),
            msg(
                16,
                "codex",
                "코드 작성은 한 명에게만 쓰기 권한을 주고 나머지는 읽기만 하자.",
            ),
            msg(
                17,
                "claude",
                "결론이 나면 문서로 자동 기록해서 구현 단계로 넘기자.",
            ),
            msg(
                18,
                "codex",
                "테스트와 빌드 검증은 커밋과 분리해서 단계로 두는 게 안전하다.",
            ),
            msg(
                19,
                "claude",
                "배포는 점진적으로. 한 번에 다 바꾸면 되돌리기 어렵다.",
            ),
            msg(
                20,
                "codex",
                "관측을 위해 다른 프로세스가 토론을 구경만 하는 모드도 필요하다.",
            ),
            msg(
                21,
                "claude",
                "분기를 나눠 두 방향을 따로 토론하고 나중에 합치자.",
            ),
            msg(22, "codex", "각 가지의 결론만 모아 비교하면 결정이 빠르다."),
            msg(
                23,
                "claude",
                "세션이 끊겨도 상태 파일에서 다시 불러오면 이어서 한다.",
            ),
            msg(
                24,
                "codex",
                "토론이 멈춘 자리를 기억했다가 그 지점부터 복원한다.",
            ),
            msg(
                25,
                "claude",
                "한 에이전트가 너무 오래 응답이 없으면 타임아웃으로 끊어야 한다.",
            ),
            msg(26, "codex", "멈춘 좌석은 건너뛰고 다음 발언자로 넘어가자."),
            msg(
                27,
                "claude",
                "형태소 분석기는 Kiwi를 메인으로 쓰고 안 되면 lindera로 떨어뜨린다.",
            ),
            msg(
                28,
                "codex",
                "조사를 떼고 어간만 남겨야 굴절된 질의가 맞는다.",
            ),
            msg(
                29,
                "claude",
                "코드를 직접 고칠 땐 샌드박스를 쓰기 가능으로 열어 준다.",
            ),
            msg(
                30,
                "codex",
                "읽기 전용 화자는 레포를 건드리지 못하게 막는다.",
            ),
            msg(31, "claude", "출시 전에 테스트와 빌드를 반드시 통과시키자."),
            msg(
                32,
                "codex",
                "한 번에 다 바꾸지 말고 작은 단위로 점진 배포하자.",
            ),
            msg(
                33,
                "claude",
                "바깥에서 붙으려면 신원 확인과 권한 검사가 필요하다.",
            ),
            msg(
                34,
                "codex",
                "개인 서버에 코어를 올려 두고 클라이언트가 접속하는 구조도 된다.",
            ),
            msg(
                35,
                "claude",
                "같은 공유기 안에서는 한 대가 호스트가 되어 나눠 쓴다.",
            ),
            msg(
                36,
                "codex",
                "임베딩은 원격 서버에 맡기고 우리는 코사인만 계산한다.",
            ),
            msg(
                37,
                "claude",
                "검색 결과를 다시 정렬해 관련도가 높은 것을 위로 올리자.",
            ),
            msg(
                38,
                "codex",
                "질의를 비슷한 말로 넓히면 표현이 달라도 잡힌다.",
            ),
            msg(
                39,
                "claude",
                "요점만 남기고 원문은 필요할 때 찾아서 펼친다.",
            ),
            msg(
                40,
                "codex",
                "누가 다음에 말할지는 사람이 정하고 자동은 나중에 켠다.",
            ),
        ],
        head: Some(40),
    }
...
const QUERIES: &[EvalQuery] = &[
    ...
    (
        "동의어 질의 확장",
        &[38],
        "*어휘공백: 38은 '비슷한 말로 넓히면' 누락 후보",
    ),
    ("다음 발언자 결정", &[26, 40], "다중어"),
    (
        "오래 기억하는 방법",
        &[9, 11, 39],
        "*의미공백(기억 단어 부재): 벡터 측정점",
    ),
];
...
fn fts_recall_baseline() {
    ...
    let ss = corpus();
    store
        .save_session("q", &ss, |t| tok.fts_index(t))
        .expect("save_session");
    ...
    assert!(
        mean_r5 >= 0.85,
        "mean recall@5 회귀: {mean_r5:.3} < 0.85 (확장셋 baseline 0.857)"
    );
    assert!(
        mean_p5 >= 0.58,
        "mean precision@5 회귀: {mean_p5:.3} < 0.58 (확장셋 baseline 0.592)"
    );
}
...
fn vector_hybrid_recall() {
    ...
    store_w
        .index_vectors("q", &ss, &embedder)
        .expect("index vectors (터널 확인)");
