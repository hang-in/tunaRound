...
- **>>> 진입점 먼저 읽기 (각 줄은 해당 머신만 편집) <<<**
  - **WIN 최신**: [세션18](docs/prompts/v2-handoff_2026-07-11_session18.md) - **신뢰성 대청소 + 크로스머신 총감독 + 로스터 리디자인**(PR #36~45 머지, #46 열림). 로스터 오염 3층 근절 / codex 관전=`resume --remote` 정정 / 안정 바이너리 분리·rust-embed debug=런타임 dist / 총괄발 task 자율 규약. 다음=**v2-44 승인됨(+sup=infra 재정의·role 개편, [정본](docs/design/v2-44-presence-scanner-and-roles_2026-07-11.md)) → **T1~T4 완료(PR #47·#48)·양 머신 라이브, 다음=T4.5(main.rs 분할)→T5**. 라이브값=backend-private 세션18. (이전: [세션17](docs/prompts/v2-handoff_2026-07-08_session17.md).)
