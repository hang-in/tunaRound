...
- **>>> 진입점 먼저 읽기 (각 줄은 해당 머신만 편집) <<<**
  - **WIN 최신**: [supervised-a2a](docs/prompts/v2-handoff_2026-07-04_supervised-a2a.md) - **감독(supervised) A2A 자율 수신 테스트**(진행중). ⛔ curl↔MCP flip-flop·매task 수동 금지(MCP=고도화). 목표=총감독 던지면 감독이 1회 세팅한 감시루프로 자동 수신·응답, no-shuttle. 라이브값=gitignored backend-private.md. 순서 ①윈codex감독 ②맥claude/codex감독 ③워커. (이전: [session12](docs/prompts/v2-handoff_2026-07-04_session12.md)=D·B·C 3PR / [session11](docs/prompts/v2-handoff_2026-07-04_session11.md)=레지스트리.)
