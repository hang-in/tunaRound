...
> **배정 → 위임 (책임의 이전).** 위 위계가 "누가 뭘 하냐"라면, 위임 행동 규약은 "총괄이 손을 떼도 되냐"다. 관리자·실무자는 루틴 문제를 자가 처리·재시도하고 terminal(completed/failed)로 반드시 보고하며, 막힐 때만 (무엇이/왜 막혔는지/뭐가 필요한지)를 명확히 에스컬레이션한다. 총괄은 매 task를 폴링하지 않고 `watch-results` 인박스로 결과만 받는다. 규약·task 프리앰블 템플릿=[a2a-usage §12·§13](docs/reference/a2a-usage.md).
...
## 현재 상태 (2026-07-11, 세션 6~20. 세션20 = 재부팅 복구 1줄 + v2-46 codex-relay 완주 + T5 정리)

- **세션 20 (2026-07-11): 재부팅 복구 스크립트 + v2-46 codex-relay(sup 폐기) 양 머신 완주 + T5 정리 + 운영 견고화 A.** ① **PR #53**: restart-win-mesh.ps1(재부팅 복구 1줄) + presence 스냅샷 10초 데드라인(이 머신 tasklist 무한 행 실측 = 로스터 win 전멸의 근본 원인) + 훅 Monitor 경로 정규화(exit 127). ② **PR #54 = v2-46**: codex-relay 데몬(로컬 codex 세션 대리 claim → 보이는 세션 thread 직주입, 실패=fail_task) + codex-inject --thread + GoalForm codex 카드 복귀 + **유령 B석 tombstone**(.ctx 삭제→"dead" 기록, 삭제=보수적 유지의 자기모순 해소). E2E win 403·mac 실증, **mac 이관 = A2A task 자율 수행**. 세션19 §4 win attach 타임아웃 = 해소 관찰. ③ **PR #55 = T5**: supervised alias 제거·candidates/discover 전삭제(스캐너가 대체)·a2a-usage §10 재작성+infra 태그 규약 명문화. ④ **PR #56 = 운영 견고화 A**: mesh.pids 선별 종료+rename-swap(**재기동해도 세션 수신 poll 생존** 실측 - luckyCAD exit 127 근본 해소) + task get 열린 task 본문 + 피드 uuid→이름. ⑤ **조사 확정 2건(B 입력)**: 피드 리로드 전멸=이벤트 백필 없음 / ★ 결함=human_input_at 인메모리(재기동마다 증발)+codex 입력 무신호. ⑥ 부수: statusline 복원(시스템 python), tunaCTX 삭제=의도(메모리 기록). **다음 = B(v2-45: watch-results 재접속·재생, 피드 스냅샷, ★ 영속·codex 신호, mesh 기억화, Redis opt-out) → C.** 진입점 [세션20 핸드오프](docs/prompts/v2-handoff_2026-07-11_session20.md).
...
- **>>> 진입점 먼저 읽기 (각 줄은 해당 머신만 편집) <<<**
  - **WIN 최신**: [세션20](docs/prompts/v2-handoff_2026-07-11_session20.md) - **재부팅 복구 1줄(restart-win-mesh.ps1) + v2-46 codex-relay 양 머신 완주 + T5 정리 + 운영 견고화 A**(PR #53~#55 머지, #56 CI 중). 유령 B석 tombstone·세션 poll 재기동 생존 실측. 다음=**B(v2-45: watch-results 재접속·재생, 피드 스냅샷, ★ human_input_at 영속+codex 신호, mesh 기억화, Redis opt-out)**. 라이브값=backend-private 세션20. (이전: [세션19](docs/prompts/v2-handoff_2026-07-11_session19.md).)
  - **MAC 최신**: [mac-autoarm-philosophy](docs/prompts/v2-handoff_2026-07-08_mac-autoarm-philosophy.md) - 맥: win 위임 task 7건 자율 처리(char-패닉 실버그 PR #21 머지 / 거버넌스 정제 PR #31 채택 / 아키텍처 리뷰 / autoarm 전면화 / discover 상주 240분) + **제품철학 정립**(개인 악기·기억=moat·역할모델·계보, 메모리 3개). **콜드스타트**: autoarm 훅으로 새 세션 자동무장(presence), 라이브 감독은 Monitor로 poll 재-arm(uuid=session-id). **재부팅했으면 `bash ~/.tunaround/restart-mac-mesh.sh` 먼저**(nohup 데몬 codex sup·discover·app-server 다 죽으니 멱등 재기동). /exit만이면 그 데몬들은 생존. (이전 [mac-a2a-supervisor](docs/prompts/v2-handoff_2026-07-06_mac-a2a-supervisor.md).)
