...
## 현재 상태 (2026-07-11, 세션 6~18. 세션18 = 신뢰성 대청소 + 크로스머신 총감독 + 로스터 리디자인)
...
- **>>> 진입점 먼저 읽기 (각 줄은 해당 머신만 편집) <<<**
  - **WIN 최신**: [세션19](docs/prompts/v2-handoff_2026-07-11_session19.md) - **v2-44 완주 + 유령·깜빡임 근절 + 수신 완전 자동화 + codex "보이는 세션" 전환**(PR #47~#51 머지, #52 대기). 정본 재확정 5개(핸드오프 §2) + 감독발 task 승인 게이트 폐지. 다음=**PR #52 머지 확인 → T4.7(codex-relay: sup 정체성 폐기·세션 thread 주입)**. 라이브값=backend-private 세션19. (이전: [세션18](docs/prompts/v2-handoff_2026-07-11_session18.md).)
