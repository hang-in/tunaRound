...
- **실무자** (헤드리스 워커, role=worker): **1 task → 1 브랜치/worktree.** worktree로 물리 격리(병렬 충돌 원천 차단). 무관 파일 손대지 않고, 끝나면 보고한다.

**핵심 규칙 5개**:
1. **비trivial 변경 = 브랜치.** main 직접 편집 금지, main = 총괄 머지 전용(CI green 전제).
...
4. **미커밋 들고 있지 않기.** 코드 변경은 곧 브랜치 커밋(라이브 반영 ≠ 커밋). 스테일 브랜치는 정리한다.
