...
> **배정 → 위임 (책임의 이전).** 위 위계가 "누가 뭘 하냐"라면, 위임 행동 규약은 "총괄이 손을 떼도 되냐"다. 관리자·실무자는 루틴 문제를 자가 처리·재시도하고 terminal(complete/fail)로 반드시 보고하며, 막힐 때만 (무엇이/왜 막혔는지/뭐가 필요한지)를 명확히 에스컬레이션한다. 총괄은 매 task를 폴링하지 않고 `watch-results` 인박스로 결과만 받는다. 규약·task 프리앰블 템플릿=[a2a-usage §12·§13](docs/reference/a2a-usage.md).
