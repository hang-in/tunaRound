...
fn main() {
        ...
        rt.block_on(async {
            ...
            for l in auto {
                ...
                handles.push(async move {
                    let run = async {
                        let runner = cli_node::build_lane_runner(&l, &token)?;
                        ...
                        let client = cli_node::connect_with_retry(&core_url, &token, 20).await?;
    ...
    #[cfg(feature = "sqlite")]
    let annotation_sink: Option<Box<dyn tunaround::orchestrator::AnnotationSink>> = match &db_path {
        Some(p) => match tunaround::store::sqlite::SqliteStore::open(p) {
