// 러너 브랜드 아이콘(claude=Claude 스타버스트, codex=OpenAI 매듭). 경로 출처: lobe-icons(static-svg).
// 알 수 없는 러너는 이니셜 색점으로 폴백한다. 색은 로스터 VALUE_COLOR와 동일 계약.
...
export function RunnerIcon({ runner, size = 15 }: { runner: string | null; size?: number }) {
  const name = runner || '?'
  const path = name === 'claude' ? CLAUDE_PATH : name === 'codex' ? OPENAI_PATH : null
  if (path) {
    return (
      <svg
        width={size}
