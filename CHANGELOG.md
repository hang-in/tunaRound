...
- 좌석(seat) 수신함: `send_task`의 `to_agent`에 `mbox:machine=<m>,project=<slug>` 좌석 주소를 쓰면 지금 그 자리에 세션이 없어도(offline) 작업이 durable하게 대기했다가, 그 머신·프로젝트에서 열린 세션이 집어갑니다. 세션 수신 루프가 자기 uuid와 좌석 주소를 함께 감시하고(`poll --also-agent`, 훅이 자동 배선), 발신자는 send 직후 `get_task`의 submitted 잔류로 미배달을 확인합니다(규약 상세는 a2a-usage 문서).

### Changed

- 내부 의존성 메이저 정리 2차: rusqlite 0.40(번들 SQLite 3.53 - 실데이터 사본 마이그레이션 리허설로 무손실 검증), tokio-tungstenite 0.30, rmcp 2.2(MCP 툴 스키마 20종 등가와 구버전 클라이언트 왕복 하위호환을 실측 검증). 서드파티 라이선스 고지를 재생성했습니다.

### Fixed

- Claude Code 백그라운드 데몬(bg 슬래시 실행)이 옛 세션을 fork-resume하면 그 세션이 대시보드 로스터에 유령으로 나타나던 문제를 고쳤습니다(#161). presence 스캐너가 데몬 roster를 읽어 bg fork 세션과 되살아난 옛 세션(마커 없는 경우 한정)을 제외하고, 같은 세션이 실제로 열려 있으면 그대로 표시합니다. 데몬 파일이 없거나 포맷이 다르면 기존 동작을 유지합니다.
- 대시보드 다크모드에서 스크롤바가 밝은 채로 남던 문제를 고쳤습니다(테마별 color-scheme 선언 + 스크롤바 색을 팔레트 토큰에 연동).
- 쓰기 워커의 자기훼손 방지 가드가 상대경로 프로젝트를 노드 기준이 아니라 프로세스 실행 위치 기준으로 판정할 수 있던 문제를 고쳤습니다.
...
- `get_task`에 `wait_secs`(1~120) 옵션이 생겼습니다. 지정하면 작업이 끝날 때까지 서버가 대기(long-poll)했다가 반환하므로, 위임한 쪽이 폴링 간격을 관리할 필요가 없습니다. CLI에서도 `tunaround task get <id> --wait <초>`로 쓸 수 있고, 내장 클라이언트가 long-poll 요청에 한해 타임아웃을 자동 상향해 긴 대기에도 클라이언트가 먼저 끊지 않습니다.
...
- 러너 실행 시 stdin 프롬프트 주입이 실패해도 성공이나 무출력 타임아웃으로 위장되던 문제를 고쳤습니다. 이제 주입 실패에 자식 비정상 종료가 겹치면 IO 오류로 정확히 보고됩니다(입력을 다 읽지 않고 정상 종료하는 러너는 실패로 오탐하지 않습니다).
- 대시보드 견고화: roster가 내부 시각 조회 실패를 정상 응답으로 위장하지 않고 500으로 표면화하고, 등록해제의 내부 오류를 미등록(404)과 구분하며, 목표 제출에 같은 대상이 중복 지정돼도 작업이 한 번만 생성됩니다. 피드 재생 스냅샷이 직렬화 동안 저장소 락을 잡지 않아 대량 재생 중에도 다른 요청이 밀리지 않습니다.
