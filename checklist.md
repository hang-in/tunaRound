...
- [ ] T4.7 codex 수신 대리 재설계(사용자 지적 2026-07-11 "sup은 백엔드인데 정리 안 됨"): v2-37 잔재로 주입이 sup 사설 글루 thread로 감 → **sup=순수 배달부**로 정리: 자기 머신 runner=codex 세션들 앞 task를 대리 폴링·claim하고 **그 세션의 threadId로 주입**(글루 thread 폐지). GoalForm codex 세션 카드=유효 대상 복귀(그 머신 codex-inject infra online 조건). 관전 전제=codex를 --remote로 여는 규약(mac 충족, win 온보딩 문서화). 임시 프리뷰=mac task f3026e3f(.thread를 운영자 세션으로 교체). **착수 2026-07-11(세션20), 설계 정본=[v2-46 codex-relay](docs/design/v2-46-codex-relay_2026-07-11.md)**: `codex-relay` 데몬(enumerate_codex_live 재사용, 대리 claim, in-process 주입, 실패=fail_task) + codex-inject `--thread` 직지정 + GoalForm 재정의 + sup·핸들러 폐기.
- [ ] (T4.7과 같은 PR) 유령 B석 근절: SessionEnd의 .ctx **삭제 → tombstone("dead") 기록**으로 교체 + 스캐너 `MarkerState::Dead=즉시 제외`. 원인 실측(2026-07-11 세션20): 깨끗이 종료한 세션 = 마커 없음(NoMarker=보수적 유지) + jsonl mtime 신선 → 240분 유령(f09e84dc·46a26152). 마커 삭제가 "즉시 제거"를 오히려 무력화하던 자기모순 해소. resume 시 write_marker가 pid로 덮어써 부활 정상.
