...
## 외래어 음역 병기 색인 (2026-07-02 세션5 착수)
> 근거: 실코퍼스 확장이 리프레시↔refresh 갭 노출, 하이브리드(임베딩)도 못 메움(반증 실측). 임베딩은 의역/동의어는 잇고 음역+영어term조밀만 못 이음 → 어휘층 alias 병기가 직접 해법. 결정: 음역 페어 집중(의역/번역 제외=임베딩 담당), 질의 시점 양방향 확장(index 무변경=재색인 불요).
- [ ] search/mod.rs: LOANWORD_GROUPS(음역 페어, 소문자·모호단음절 제외) + loanword_aliases(token)→Vec<alias> + 단위테스트
- [ ] tokenizer.rs fts_query(default trait): 질의 토큰별 alias 추가 후 dedup(모든 백엔드 공유)
- [ ] main.rs 비morphology fallback fts_query에도 동일 확장(일관성)
- [ ] 재측정: real_corpus_recall FTS + hybrid. "리프레시 토큰 어디 저장" R@5 0→>0 확인, mean 변화·P 훼손 여부. floor 재조정
- [ ] ⚠ 정밀도 영향(alias가 OR 확장) 측정·명시. 자동음역 모델 비채택(개인도구 과함)
