...
- [ ] 마감 잔여: win mesh 재배포(rmcp+tungstenite+rusqlite+스크롤바+훅 3파일, tungstenite relay 라이브 확인) → mac은 구버전 호환 실측됐으므로 v0.7.0 릴리스 때 정렬(⚠ mac 훅 배포는 새 바이너리 선행 - 구 poll에 --also-agent 없음) → v0.7.0 릴리스 여부 사용자 결정
