...
- **#147 좌석 수신함 계약(이슈 코멘트=정본)**: `mbox:machine=<m>,project=<slug>`(machine 필수) / pickup=uuid+좌석 이중폴(서버·스키마 0) / first-claim CAS=좌석 내 정상 / **발신자 보상 체크 필수**(send 후 get_task submitted 잔류=미배달 신호) / Stage 2(비actionable ack·TTL sweeper)=2번째 실사례 트리거. **machine 파생의 정본 = rust 함수가 아니라 배포 파이프라인 실효값**(restart 스크립트가 config→env 주입 → 스캐너 실효 우선순위 config>env>OS) - 훅 py cfg-first는 그 정렬이다(적대 검증 FAIL을 이 논증으로 소거, PR #154 코멘트).
- **C-④ 처방(토론 합의)**: b(동일-초 통째 페이지)는 "초 단위 워터마크+`>=`인 한 단일 초가 LIMIT을 채우면 전진 불가"로 근본 불가 증명·기각. c(wedge 로그, MAX+1 peek 접기)=구현 완료(#150), a(keyset (updated_at,task_id)+since_id+rowid→task_id tiebreaker+§5 재검증)=**c 로그 첫 발화가 착수 트리거**(막연 백로그 아님). 번들 금지.
- **의존성 메이저 3종 실측 총평**: rusqlite 0.40=코드 0(표면이 좁으면 9단계 점프도 무변경), tungstenite 0.30=1줄, rmcp 2.2=기계 치환 57지점. "메이저=큰 작업" 가정은 이번에도 과대. 단 rmcp급은 **스키마 스냅샷 오라클 선행 커밋 + 구클라 라이브 왕복**이 등가 증명의 핵심이었다.
- **공유 CARGO_TARGET_DIR 병렬의 함정 2건**: ① 교차 오염된 테스트 바이너리를 cargo가 fresh로 오인 실행(카운트 왜곡 - touch 리빌드로 재측정) ② 백그라운드 빌드 중 브랜치 전환=혼합 트리 산출물. 병렬 에이전트 검증·전용 산출물은 **에이전트별/목적별 전용 target dir**로.
- **대시보드 다크 스크롤바 교훈**: CSS 변수 팔레트만으론 브라우저가 여전히 라이트 문서로 취급한다 - 테마 전환 UI에는 **color-scheme 선언이 필수 짝**(네이티브 위젯 전부에 파급). 검증은 Chrome CSSOM(computed colorScheme·scrollbarColor)+확대 스크린샷.
- **mac 훅 배포 순서 제약**: 좌석 수신함 훅(--also-agent)은 신 바이너리 전제 - 구 poll은 그 플래그를 모른다. mac은 v0.7.0 정렬 때 바이너리→훅 순으로(구클라↔신서버 wire 호환은 실측 완료라 그때까지 mesh 혼용 무해).
...
- **(07-19) "기타" uuid 유령 클래스 종결**: 훅 수정은 새 세션만 고친다 - **훅 배포 이전에 무장된 장수 세션의 Monitor는 소급 안 됨**(mac home 세션 poll 3일 생존 실측). 잔재는 세션 재시작 또는 원격 task로 자가 재무장. src 소유권 가드는 무태그 수동 등록이 이기면 스캐너 복원을 막으므로, 정리 순서=poll 교체 후 deregister(스캐너가 재소유).
- **(07-19) #136 교훈 3개**: ① 위임 본문은 배달 프리뷰에서 잘린다 - **발신=요지 선두 160자 / 수신=claim 전 get_task 전문 확인**이 규약(훅+a2a-usage 배포됨). ② 워커 보고도 오진 가능 - "도구가 안 준다"는 보고는 자기 실측(프로브)으로 교차 검증 후 믿기(실제 원인=head -1 절단·completed 대상 테스트). ③ completed task는 의도적으로 result만 반환(요청 전문은 working 동안 읽어야).
- **(07-19) #133 방법론**: 시한폭탄 판정 기준 = 고정 리터럴 × 실벽시계 삭제/필터의 **트리거 경로** × 테이블 의존 단언 3교집합(인메모리 단언은 무관). 단일 파일 분류기는 크로스 파일을 놓친다 - 적대 스캔이 server.rs 1건 추가 발견.
