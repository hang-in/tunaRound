...
**★ 재부팅 후 첫 세션이면 이 한 줄부터**(2026-07-08 추가): `bash ~/.tunaround/restart-mac-mesh.sh`. /exit와 달리 **재부팅은 nohup 데몬(app-server·codex-sup poll·discover)까지 다 죽인다**. 스크립트는 멱등(이미 떠 있으면 skip, 죽었으면만 기동)이고 토큰은 env 폴백이라 argv 노출 0. presence(로스터 online)는 새 세션 autoarm 훅이 자동 처리하므로 스크립트 없이도 뜨지만, **감독 mesh(codex 관전·discover 후보·app-server)는 이 스크립트가 있어야 복구**된다. 아직 launchd 미도입이라 부팅 자동기동은 안 됨(④ 미완 항목). git/커밋은 재부팅과 무관(원격 push 완결 상태로 종료).
