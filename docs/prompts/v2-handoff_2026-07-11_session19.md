...
v2-44 전 구간(T1~T4.5) 구현·라이브 전환 완료(PR #47~#52 전부 머지). 유령 세션·로스터 깜빡임을 근본 원인 3개(마커 부재·tasklist 부하 에러·수신 poll 무태그 재등록)까지 파서 근절, 수신을 완전 자동화(사용자가 A2A를 언급할 필요 0), codex 주입을 "보이는 세션"으로 전환(mac 실증). 승인 게이트 폐지 등 거버넌스 개정. 다음 = T4.7(codex-relay).
...
1. `git pull`(#52까지 전부 머지됨=5f16931).
