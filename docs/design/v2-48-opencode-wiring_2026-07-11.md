...
>
> **세션24 재대조·확증(2026-07-12, v0.4.0 릴리즈 후) - 3축 병렬 정찰 결과 = still_churning, defer 유지**: ① **로컬 스키마(v1.17.18 실측) = GO** - 스캐너가 의존하는 §1의 6컬럼(id·project_id·directory·title·time_created/updated)이 이름·타입 불변(전체 `session` 테이블은 29컬럼으로 성장했으나 이 6개는 그대로), read-only 열거 SQL이 15세션 반환, `role`은 여전히 `message.data` JSON 안. 가산 드리프트만(신규 컬럼 다수 + 신규 `session_message` 이벤트소싱 테이블 4행, 스캐너 열거 질의엔 무영향). ② **CLI/API = GO** - `opencode serve` 6엔드포인트(`POST /session/:id/prompt_async` 204 등)·`--port/--hostname`·`chat.message` 훅·MCP remote+Bearer 전부 유지(사소 정정: 훅 인자는 `input.sessionID/messageID`, serve `--password` 플래그 없음=env 전용). ③ **업스트림 = NO-GO(still_churning)** - 마지막 릴리즈 07-09에서 3일(몇 달 아님), 캐던스 여전히 거의 매일. 결정적 OPEN 이슈: **#36222**(설치 버전 v1.17.18 업데이트 후 세션 사이드바 소실·DB엔 존재·downgrade `no such column: data` = R2의 조용한-0 실패가 설치 버전 상대로 실재), **#34922**(maintainer chore: "review V2 table and durable event type names **before GA** ... before any migrations land" = 스키마 소유자가 V2 테이블/컬럼/이벤트타입 이름을 pre-GA·리네임 예정으로 명시), + 세션 가시성 클러스터 #36064·#36464·#36178. **재검토 트립와이어(3조건 전부 충족 시 재대조)**: ⓐ #34922 종료(V2 스키마 GA/네이밍 확정) + ⓑ 가시성 클러스터(#36222·#36064·#36464·#36178) 해소 + ⓒ 릴리즈 캐던스가 주 단위 이상으로 지속 둔화. 충족 전 감독 레인 미착수. **착수 시 scanner 권고**(로컬 실측 기반): cwd는 `directory` 컬럼 사용(신규 `path` 아님), 라이브 한정은 `WHERE time_archived IS NULL`(신규 nullable 컬럼). ④ **워커 레인 신설 caveat**: **#33320**(OPEN 07-10) 동시 `opencode run` 워커가 startup에서 `database is locked` 실패 가능 → 한 머신에서 다중 opencode 워커를 동시 실행할 때 잠재 실패(배포된 워커 레인 영향, 현재 단일 워커 운용이면 무해).
