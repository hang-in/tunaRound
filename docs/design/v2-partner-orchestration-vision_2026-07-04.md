...
3. **에이전트 등록·발견·능력 라우팅(UUID+태그)**: 수동 문자열 id를 UUID(라우팅)+태그(발견)로 교체. dispatcher가 태그 셀렉터로 파트너를 발견해 라우팅. 정본 [등록·발견·능력 라우팅](v2-agent-registry-uuid-tags_2026-07-04.md)(다음 세션 구현). 거버넌스 §6 흡수.
4. **소환 UX**: 감독 세션이 "이 일 누구한테 시킬까"를 매끄럽게 고르고 던지는 인터페이스(현재는 raw send_task). 위 로스터/발견 위에 얹음.
