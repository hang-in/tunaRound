...
**tunaRound 포팅 방침:** secall 정본 `tokenizer.rs`(Kiwi 메인 + lindera 폴백 + factory) 통째 포팅, `backend="kiwi"` 기본. 그 위에 BGE-M3 벡터 + 하이브리드 단계적. 의존성 무거우니(kiwi-rs/lindera/ort/ndarray/tokenizers/usearch) feature-gate(`morphology`, `friend-engine-semantic` 패턴).
