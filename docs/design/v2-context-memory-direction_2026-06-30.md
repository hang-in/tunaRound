...
**임베딩 = 원격 Ollama (결정 2026-06-30, 로컬 ORT 대체 / 검증됨):** BGE-M3를 로컬 ONNX(ort/ndarray/tokenizers)로 돌리지 않고, **지인 서버 Ollama를 SSH 터널로** 쓴다(사용자: 로컬 bge-m3 쓰지 말 것).
- **검증(2026-06-30):** `ssh -N -p [사설포트] -o StrictHostKeyChecking=accept-new -L 11435:127.0.0.1:11434 [사설계정]@<host>` 로 터널 OK. 원격에 **`bge-m3:latest` 존재**(+ gemma4:e2b/e4b). `POST http://127.0.0.1:11435/api/embed` `{"model":"bge-m3","input":"..."}` -> `{"embeddings":[[...]]}`, **차원 1024**. (**SSH 포트 = 2232**, IP 비공개.)
- tunaRound의 `Embedder` = **reqwest HTTP**(OrtEmbedder 대신) + `MockEmbedder`(결정적 폴백/테스트/터널 다운 시). 엔드포인트는 설정값(기본 `http://127.0.0.1:11435`). 장점: 무거운 ONNX 의존 제거(reqwest만), 연산 오프로드.
- 주의: (1) 터널 떠 있어야 동작 -> 다운 시 graceful(에러 또는 Mock 폴백), (2) **이 Claude 세션 환경에선 터널이 되지만(포트 2232), 일반적으로 사용자 네트워크 의존**, (3) 호스트/IP 비공개. 의존성 feature-gate(`morphology` = kiwi-rs/lindera, `semantic` = reqwest 임베딩 클라이언트). 벡터 스키마 차원 = **1024 고정**.
