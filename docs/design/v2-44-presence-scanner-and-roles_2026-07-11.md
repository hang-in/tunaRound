...
## 10. T1+T2 구현 노트 (2026-07-11, PR 단계에서 확정된 조정)

- **W1 근본 원인 실측 확정**: 훅이 3중 등록이었다 - 전역 settings에 `python` 항목 + 별도 `python3` 항목(체인 아님) + 프로젝트 `.claude/settings.json`. 조치 = 프로젝트 등록 제거(레포) + 마커 파일 1회 보장(코드) + 전역 이중 엔트리 정리(T2 ops).
- **프로세스 교차 확인은 러너 카운트만**(MVP): `tasklist`/`ps`로 러너 프로세스 수를 세어 0개면 그 러너 세션 전부 제거. per-session 매핑(sysinfo cwd)은 신규 dep·플랫폼 편차라 후속. 개별 크래시 유령은 신선도 창(240분)이 상한.
- **SessionEnd 훅은 deregister 핑 1줄 잔존**(§7 부수 결정 "제거"에서 조정): 깨끗한 종료를 스캔 주기 대기 없이 즉시 반영 + 구식 detached poll 전환기 정리(kill+pidfile). 무장·리핑 코드는 전부 삭제됨.
- **codex 세션 인식 = rollout `session_meta`의 `originator=codex-tui`만**(exec 헤드리스 제외 = 로스터=열린 TUI 세션 계약). 래퍼(scripts/codex 3종)는 삭제.
- **스캐너 소유 격리 = `src=scan` 태그**: 일괄 제거는 이 태그가 있는 항목만. 수신용 수동 poll이 같은 uuid를 재등록해 태그를 덮어도 다음 스캔(≤15초)이 복원한다.
