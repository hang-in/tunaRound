...
> 지위: **정본**(2026-07-18 사용자 발의 → 같은 날 "완료까지" 지시로 착수). 3렌즈 적대 검증 1회 + **Phase 0 게이트 통과**(§4a 실측 기록) 후 §8 결정을 확정하고 Phase 1을 구현했다(PR 참조).
>
...
## 4a. Phase 0 실측 기록 (2026-07-18, 게이트 통과)

총괄 세션(win)이 위 레시피로 2좌석 2라운드(3 leg) 크로스머신 토론을 실제 수행했다. 좌석 = mac-claude-tunaRound(라이브 세션, 자율 수신) + win-debate-worker(임시 헤드리스 work 데몬, ReadOnly, project=tunaRound). 주제 = 바로 이 문서의 §8-4(재기동 실패 처리 (a) vs (b)).

- **가치 판정: PASS.** 토론이 설계를 실제로 개선했다. reviewer(win 워커)가 proposer(mac)의 자기모순(인메모리 락에 릴리스 단계를 둔 것)과 과장(마이그레이션 불가역론 - 이 레포는 v4→v11을 통상 비용으로 지불해 왔음)을 잡았고, "무-open-task 수 초 창 침묵사"라는 미발견 엣지를 찾아냈다. 라운드 2에서 proposer가 조건 5개 전부 수용·최종 5줄 스펙 합의. 순차-인지(직접 인용 반박) 성립.
- **타이밍 실측**: leg당 150~200초(mac 라이브 190·150초, win 헤드리스 200초). 3 leg + 수동 조립 ≈ 15분.
- **마찰 실측(→ Phase 1 반영)**: ① 프리앰블은 좌석 유형 인지 필요 - 헤드리스 좌석은 complete_task를 직접 못 부르므로 "출력=발언" 변형이 필요(라이브는 claim/complete 지시 유지). driver의 `debate_preamble(live)`로 반영. ② /a2a는 토큰 기동 브로커에서 Bearer 필수(로컬 무토큰은 init 프리셋 구성의 계약). ③ 라운드 조립·폴링이 총괄 컨텍스트를 그대로 소모 = Phase 1 정당화.
- **토론 산출 스펙(§8-4에 채택)**: 기동 sweep = `from_agent LIKE 'debate:%' AND state IN(열린 상태 전부)` → failed, 사유 "broker restart" 명시 / 통지 = 라운드 task의 failed terminal 자체(watch-results --dispatcher debate:<id>) / 늦은 완료 = 기존 terminal 가드로 차단 / 락 릴리스 단계 없음(인메모리 자연 소멸) / 침묵사 창은 수용 잔여 리스크로 문서화(악화 시 debate-liveness 1행 표적 확장, 전면 영속 (b)는 '재개'가 실 요구일 때만).
...
## 8. 확정 결정 (2026-07-18 착수 세션, 사용자 "완료까지" 위임 하에 권고안 채택)

1. **인박스 배달 = (a) 확정.** 라운드 task from_agent = `debate:<id>`, 총괄이 `watch-results --dispatcher debate:<id>`를 추가로 띄운다(라운드마다 RESULT 줄, 마지막 완료=종합). start_discussion 응답이 정확한 명령을 안내한다.
2. **대시보드 검색 `debate/*` = MVP 비노출 확정.** 실효 범위는 과거 검색성뿐(§7-5). 유보 비용 0.
3. **라이브 좌석 = `live: true` 명시 필수 확정.** resolve_seats가 로스터 role=session 좌석에 강제한다.
4. **재기동 실패 처리 = (a) 고아 sweep 확정**(Phase 0 토론 산출 스펙 그대로, §4a). `fail_orphan_debate_tasks`를 기동 housekeeping에서 backfill보다 먼저 실행(방금 실패분도 같은 기동에 색인). 재기동 후 discussion_id 문의 계약 = 존재하지 않음(레지스트리 인메모리).
5. **synthesizer = 첫 좌석 고정(MVP) 확정.** 지정 파라미터는 필요가 실측되면 후속. 실패 시 합의문 없이 부분 전사 유지(failed terminal이 곧 통지).
6. **v2-55 번호 주석 정리 = 착수 PR 동승 완료**(server.rs busy 주석을 이슈 #94 지칭으로 정정).
7. **CLAUDE.md 갱신(총괄 경유) = 머지 후 수행.** 세션12 "위임 vs 회담" 요약 줄 개정 + 스테일 포인터(a2a-usage §12·§13) 정리.
...
## 11. 후속: 라운드 간 사람 승인 게이트 (이슈 #131, 2026-07-18 세션31)

첫 라이브 E2E 토론(12049ab3a0e4)의 합의 산출 1순위를 구현했다. 설계 3렌즈 적대 검증(코드 정합·결정 정합·스코프, 전부 GO_WITH_FIXES) 반영본이며 major 3건이 설계를 실변경했다.

- **표면**: `start_discussion(gate: true)` 옵트인(기본=현행 자동 진행) + `continue_discussion(discussion_id, steer?, conclude?)`.
  - steer = 전사 `debate/user` 턴(`[사용자 조향 지시]` 프리픽스)으로 append + prior 포함. 순차-인지 메커니즘 재사용(프롬프트 수술 없음)이고, prior 렌더에 화자 권위 구분이 없으므로 프리픽스가 지시성을 명시한다(스코프 렌즈).
  - conclude = 남은 라운드 생략·synthesizer 직행. 근거 = v1 원칙 "사용자가 충분하다고 판단할 때 결론을 정리"(tunaRound-v1-design:101-103)의 게이트 구현.
- **게이트 지점**: 각 성공 라운드 뒤(마지막 라운드 뒤 = 종합 발행 직전 포함, "synthesizer도 라운드"라는 stop 계약과 일관). 전 좌석 실패(aborted) 라운드 뒤에는 게이트 없이 기존 중단 경로(코드 렌즈).
- **대기 중 통지 3중**: ① 전사 게이트 마커(`debate/driver`, 다이제스트를 놓쳐도 read_transcript로 상태 재구성) ② **대기 표식 task**(driver 자가 claim으로 working 유지 + lease 5분 연장 - 브로커 재기동 시 기존 고아 sweep이 failed 전이 = 인박스 통지. "게이트 대기 중 open task 0 = §8-4 수 초 창의 무한 확장" 반증을 기존 프리미티브 0추가로 봉합, 스코프 렌즈 major. 해제 시 completed, stop 시 failed) ③ 다이제스트 task(발행 즉시 자가 claim→complete = watch-results push. 선두 줄 160자 = RESULT 프리뷰 계약).
- **승인 주체=사람 문구 계약**(결정 렌즈 major): debate 발신 클래스는 자율 수행 규약 대상이라, 다이제스트가 명령형 메뉴면 인박스 소비 세션(LLM)이 스스로 continue를 불러 게이트를 무력화한다. 다이제스트 본문·툴 설명·start 응답 모두 "사용자에게 보고하고 지시가 있을 때만 호출(자율 진행 금지)"을 명시하고 문구 계약 테스트로 고정. a2a-usage §8.5 + §8.3 예외 명문화.
- **실패 정책**(코드 렌즈 major): 다이제스트·표식 발행 실패 = store 병증 클래스 → 전사 실패와 동급으로 토론 중단. Waiting 무통지 대기는 금지이고, 자동 진행 폴백은 사용자가 명시한 승인 게이트의 조용한 제거라 비채택.
- **레이스**: Waiting을 다이제스트 발행 전에 세운다(continue 선착 거부 제거) / stop 선행 후 continue = 거부(모순 성공 응답 방지) / 늦은 완료·try_fail 경합은 기존 terminal 가드 그대로.
- **색인**: 게이트 task(from `debate:%` + to `debate-driver`)는 전면 비색인(다이제스트 요약 = 이미 색인된 좌석 발언 스니펫의 중복 축적 방지. driver 직접 try_complete는 라이브 색인 경로가 아니라 backfill 색인 대상이었다는 코드 렌즈 정정도 이 제외로 무의미해짐).
- **수용 잔여(MVP)**: 게이트 대기 타임아웃 없음(stop이 탈출구) / 대기 중 재기동 = 토론 소멸(표식 failed가 인박스 통지, continue의 "진행 중 토론 없음" 에러가 이중 발견 신호) / 대기 중 동시 1건 슬롯 점유 = 새 start 거부(거부 문구에 게이트 대기 상태·해제 명령 표기) / 절전 등으로 lease(30분) 만료·재claim이 3회 누적되면 expire sweep의 attempt 상한 격리로 표식이 조기 failed 통지될 수 있음(토론 자체는 계속 대기 - 표식 문구를 재기동 단정 없이 완화, 도그푸딩 관찰 후 재발행 로직 여부 결정) / continue 직후 stop이 겹치면 stop 우선(Proceed 폐기는 로그로 관측).
- **비채택**: 게이트 타임아웃 자동 진행(사람 부재 전제인 인박스 모델과 상충) / add_rounds / 대시보드 게이트 UI(관제탑 원칙) / 게이트 상태 영속(§8-4 비영속과 일관 - 표식 task가 재기동 통지를 대신함).
- **라이브 검증(2026-07-18, 머지 당일 도그푸딩)**: 토론 4건으로 전 경로 실증 - ① `af66e8e2a012`(실주제=#115 마이그레이션 순서, 2R+게이트 2회+steer 조향이 라운드 2 산출을 실변경+종합) ② `62337010a2d4`(conclude=라운드 2·3 생략, 전사 6턴) ③ `c4202ae619b2`(대기 중 stop=표식 failed 인박스 통지+중단 마커) ④ `0e396008e062`(**게이트 대기 중 브로커 재기동 → 기동 sweep이 표식 failed 전이 → watch-results 재접속 replay로 "broker restart" 인박스 배달** = 침묵사 봉합 실증). 에러 표면 전수(대기 중 start 거부의 게이트 힌트·id 불일치·steer 4001자 거부·재기동 후 소멸 에러)·다이제스트 색인 제외·피드 노출 확인.
