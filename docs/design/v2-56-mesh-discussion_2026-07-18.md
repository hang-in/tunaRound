...
- **수용 잔여(MVP)**: 게이트 대기 타임아웃 없음(stop이 탈출구) / 대기 중 재기동 = 토론 소멸(표식 failed가 인박스 통지, continue의 "진행 중 토론 없음" 에러가 이중 발견 신호) / 대기 중 동시 1건 슬롯 점유 = 새 start 거부(거부 문구에 게이트 대기 상태·해제 명령 표기).
