...
## 3.1 discover 커버리지 커플링 (중요)

discover의 `--stale-mins`(리포트 "잊기 지평")는 **대시보드 유휴 임계값(60분)보다 커야** 유휴 세션이 발견됨에 뜬다. discover가 이 창 밖 세션을 아예 리포트 안 하므로(discover.rs stale 스킵), stale-mins가 60분 미만이면 유휴 세션이 발견됨에도 안 뜬다(세션17 실측: 옛 기본 10분 → 10분+ 유휴 세션 전멸). **기본을 240분(4시간)으로 상향**(main.rs DiscoverArgs). 활성/유휴 분리는 프론트가 age로 하고, discover는 후보를 넉넉히 공급만 한다. 크로스머신: 각 머신의 discover가 자기 세션을 같은 창으로 리포트해야 그 머신 유휴 세션이 보인다.
