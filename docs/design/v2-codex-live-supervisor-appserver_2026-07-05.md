...
- `codex resume <threadId> --remote ws://IP:PORT`로 사람이 그 라이브 thread에 TUI 접속(HITL 가시). (정정 2026-07-10: plain `--remote`는 새 thread를 만든다 - §11.)
...
2. **사람 TUI(선택, HITL)**: `codex resume <threadId> --remote ws://127.0.0.1:PORT`로 글루 thread에 붙어 대화를 관전/개입(§11). 안 붙어 있어도 감독은 동작(무인 티키타카는 §5.2 승인정책에 의존).
...
- 사람은 `codex resume <threadId> --remote ws://...`(또는 id 모르면 `codex resume --remote ws://...` picker)로 그 thread에 붙는다. ~~**열린 질문(§7)**: `--remote` TUI가 특정 threadId를 선택/자동부착하는 UX는 라이브 확인 필요.~~ (§11에서 해소: plain `--remote`엔 부착 UX가 원래 없고, 부착은 resume 계열이 담당.)
...
- **남은 라이브 확인(ws 단계, T2·T5)**:
  1. `codex --remote ws://`가 글루-소유 threadId를 선택/부착하는 UX(관전). → **§11에서 해소**(부착은 `codex resume <threadId> --remote`가 담당, plain `--remote`는 새 thread).
...
- **codex 라이브 관전 = `codex resume <threadId> --remote ws://...` (유지, 제거 안 함).** codex 네이티브 TUI로 app-server의 글루-소유 thread에 붙어 추론을 실시간으로 본다. 이게 codex 관전의 주력 경로다(대시보드 스트림보다 낫다: 네이티브·즉시·codex 자체 기능이라 우리 유지비용 0). "--remote 제거" 스펙은 **비채택**. (정정 2026-07-10: plain `codex --remote`는 새 thread를 만들므로 관전 커맨드는 resume 계열이어야 한다 - §11.)
