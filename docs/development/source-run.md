...
일반 사용자는 [README의 시작하기 절](../../README.md#시작하기)을 사용하는 편이 간단합니다. 이 문서는 기능 개발, 테스트, 디버깅, 피처 조합 확인, 릴리스 전 검증을 다룹니다.
...
릴리스 바이너리에는 대시보드 SPA가 포함됩니다(v0.5.0부터). 소스 빌드에서 대시보드를 쓰려면 먼저 프런트엔드를 만든 뒤 Rust 바이너리에 임베드해야 합니다.
