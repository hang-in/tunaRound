...
owner: d9ng
summary: LLM 코딩 흔한 실수를 줄이는 10개 행동 규율 + 선제 설계·품질 셀프리뷰(리팩토링 최소화)의 SSOT. 이 프로젝트에서만 실험 적용. CLAUDE.md는 이 문서로 링크만 두고 신규 규칙(#5/#6/#7)만 인라인.
---
...
> dsp_cad_gcs `developmentConventions.md §2`(DDD 점진) 차용 + tunaRound(Rust) 적응. "처음부터 잘 짜서 잦은 리팩토링을 피한다"가 목적. 추상 원칙이 아니라 체크 가능한 규칙으로 둔다.

### 선제 설계 5규칙

1. **새 도메인 개념은 처음부터 Rust 타입으로.** Participant/Role/Turn/Round/Message/Verdict 등을 struct·enum 으로 둔다. string 관례 신규 도입 금지(나중에 타입 격상 = 리팩토링). tunapi `RoundtableParticipant`(engine+role+instruction)가 선례.
2. **framework-independent core 경계를 처음부터.** 코어(러너·오케스트레이터)는 프론트(REPL/TUI)·transport(나중 Redis) import 0. 의존은 안쪽으로만 향한다.
3. **분기·계산은 순수함수로.** `build_round_prompt`, consensus 추출, stream 파싱 분류를 순수함수로 빼서 단위테스트한다.
4. **cross-cutting 추가 전 touch-point 매핑.** 메시지 id/parent, 세션 같은 횡단 개념은 닿는 곳(타입·영속·프롬프트·프론트)을 먼저 적고 한 번에 배선한다. 트리-ready 스키마가 이걸로 보장된다.
5. **출처 답습.** 토론·러너 거동은 tunapi/tunaFlow 실측값(순차-인지, hardening) 그대로 따른다. 자체발명 금지.

### 안티-갓파일

한 모듈 = 한 책임. 관심사가 둘 이상 섞이거나 파일이 한 화면을 크게 넘으면 분리 신호다. 갓파일 금지.

### 확장성 = 깨끗한 경계, 미리 만든 훅이 아님 (YAGNI 화해)

- v1 은 v1 이 필요한 코드만 짠다. 확장 지점 코드(미사용 trait 구현·파라미터·config)를 미리 심지 않는다.
- 다만 v1 의 경계가 이미 알려진 v2/백로그의 seam 이면, **그 경계를 주석으로 표식해 미래 편집이 인라인·삭제로 seam 을 없애지 않게 한다.** 코드가 아니라 주석이다.
- 표식 형식은 `// [v2:redis] 이 trait 경계로 transport 가 들어옴 (spec §7-2). 인라인/삭제 금지.` 같은 한 줄 주석이다.
- 가드. v1 이 어차피 필요로 하는 경계에만 단다(표식하려고 경계를 발명하지 않음). spec v2 로드맵/백로그 항목을 참조해 추적 가능하게 하고, 계획이 바뀌면 표식도 갱신한다(stale 금지).

### 품질 셀프리뷰 (커밋 전)

- [ ] 모듈/함수가 한 가지 일을 하나 (SRP). 파일이 비대하지 않나
- [ ] 코어가 프론트/transport 를 import 안 하나. 의존이 안쪽으로만 향하나
- [ ] 내부 표현이 `pub` 로 새지 않나 (캡슐화). 인터페이스만 노출하나
- [ ] 분기·계산이 순수함수인가. 가변 전역/공유 상태를 피했나 (예측가능성·사이드이펙트)
- [ ] 경계가 깨끗해 교체 가능한가. 확장 지점을 미리 만들지 않았나 (YAGNI)
- [ ] 첫 줄 한국어 헤더(#6). 이름이 도메인 어휘(Participant/Round/Verdict)인가
- [ ] 순수함수로 단위테스트 가능한가. 테스트 동반했나
- [ ] 새 개념을 string 이 아니라 타입으로 넣었나
