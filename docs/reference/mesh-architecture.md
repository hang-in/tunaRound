...
tunaRound의 A2A는 A2A 프로토콜의 **구조를 차용**해 tunaRound 인스턴스끼리 위임하는 것이 목적입니다(우리 브로커 자체는 라우팅 확장이 있어 "A2A 기반"이라 부릅니다). 방향에 따라 호환 수준이 다릅니다. **Outbound**(우리가 외부 표준 A2A 에이전트에게 던지기)는 표준 A2A 클라이언트로 동작합니다. `--runner a2a`가 외부 agent-card를 발견해 표준 `SendMessage`로 위임하고 `GetTask`로 결과를 회수하며, 독립 표준 서버 상대 왕복을 실증했습니다. 반면 **inbound**(제3자 표준 클라이언트가 우리에게 던지기)는 비목표입니다. 브로커가 라우팅 확장 필드(`fromAgent`/`toAgent`)를 요구하고 Agent Card가 인증 게이트를 두기 때문입니다. 필요해지면 표준과 브로커 사이 번역 어댑터를 별도로 둡니다.
