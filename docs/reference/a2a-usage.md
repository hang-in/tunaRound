...
**긴 지시는 요지를 선두에(#136)**: 수신 세션이 보는 `TASK` 알림 줄은 프리뷰 절단이 있으므로, 긴 작업 지시는 **핵심 요지를 선두 한두 문장(약 160자) 안에** 배치하고 상세를 뒤에 붙이세요. 게이트 다이제스트(§8.5)가 쓰는 것과 같은 계약입니다. 수신 측 전문 회수 절차는 §4.1 참고.
...
**수신 절차 주의(#136)**: 세션에 도착하는 `TASK <id> ::` 알림 줄은 프리뷰라 **본문이 잘릴 수 있습니다**. 잘린 지시로 일하지 말고, claim 전에 `get_task(task_id)`로 요청 전문을 확인하세요(브로커는 열린 작업의 원 요청 본문을 온전히 반환합니다 - 실측 검증됨).
...
4. 대기 중에도 `stop_discussion`이 즉시 듣습니다. continue 직후 stop이 뒤따르면 stop이 우선합니다(continue가 성공을 반환했더라도 진행이 취소될 수 있습니다).
