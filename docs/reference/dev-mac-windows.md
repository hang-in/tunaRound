...
### 경로
- DB/스크래치 경로는 OS마다 다름. `--db`는 상대·절대 경로 자유. 프로파일 설정파일(Stage 3, `tunaround.toml`) 도입 후엔 `~/.tunaround/…` 같은 홈 기준을 권장.
...
- **완료**: v1 + v2 검색/맥락 로드맵(step 2~8) + Stage 3a-3d + codex pull(behavioral) + 실코퍼스 회귀(step 6) + 외래어 병기 색인 + 임베딩 기본 qwen3. 배포 온보딩 **Stage 1(clap 서브커맨드)·Stage 2(cargo-dist 설정)** 완료.
- **대기/다음**: 배포 실릴리스(도그푸딩 후 태그) · 온보딩 **Stage 3(`tunaround.toml` 프로파일, 진입 선택)** · Stage 4 doctor · abstraction/anchors(보류) · 분산 코어 홈랩 호스팅(보류).
