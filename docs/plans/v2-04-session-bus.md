...
구현 완료(브랜치 `feat/v2-session-bus` -> main). 56 테스트(49 pass + 2 ignored 라이브 Redis + 통합 5), `cargo build`/`clippy` 경고 0. Opus 리뷰: 포팅 충실, 적응(room->session/env/로그) 정확. 잔여 tunaSalon 용어 주석(web/LiveSession)만 정리.

- Task 1: 의존성(tokio/redis 0.32/futures-util) + `src/session_bus.rs` 포팅 (커밋 `0783179`). redis 0.32 API가 출처와 동일해 조정 불필요.
- Task 2: 라이브 Redis 왕복 통합 테스트 2개 #[ignore] (커밋 `86aa482`).
- 리뷰 정리: 포팅 잔여 영어 주석(tunaSalon web/LiveSession 용어) 한국어로 (커밋 `11e1f52`).
- 완전 격리 확인: 기존 동기 코드 무변경, main.rs 런타임 미도입. 평소 `cargo test`는 Redis 없이 green.
