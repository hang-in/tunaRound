...
## 라이브 검증 + 버그 수정 (2026-06-30)

로컬 Redis(brew, 8.8.0) 설치 후 실 검증. 전부 통과:
- **Bus 레이어:** `#[ignore]` 3개(command 왕복 / owner acquire·refresh / snapshot set·get) 실 Redis GREEN.
- **resume(`--session`):** Redis snapshot 시드 -> 트리 복원(`/branches` 확인). async↔sync block_on 브리지 동작.
- **observe(`--observe`):** snapshot 출력 + 라이브 pubsub 이벤트 수신 확인.
- **컨텍스트 유지:** 실 3라운드(claude+codex)에서 R2가 R1 비판 반영해 입장 수정, R3가 R1 제약(SQLite) 정확 회수 + 누적 요약. 순차-인지 + 전사 누적 실증. git 청결(ReadOnly).

**발견·수정한 버그(실 라운드로만 잡힘):** mirror가 fire-and-forget이라 `/quit` 시 런타임 drop으로 **마지막 라운드 snapshot 유실**(events=3인데 snapshot=2라운드분). resume 정확성 결함. -> **수정:** 종료 직전 `session.snapshot_json()`을 동기 `set_snapshot`으로 1회 기록(`Session::snapshot_json` 추가). 1라운드 재검증으로 snapshot이 라운드를 보존함 확인. 브랜치 `fix/v2-06-snapshot-flush`(`50edea4`).
- 잔여(허용): per-round 이벤트 fire-and-forget은 best-effort(라이브 관찰자는 라운드 중 수신). resume 정확성은 종료 동기 flush로 보장.
