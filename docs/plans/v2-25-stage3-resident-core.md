...
## 3a-1 구현 스펙 (HTTP MCP 서브 모드, 첫 위임) - rmcp 1.8.0 API 실측 확정

- **rmcp API**: `rmcp::transport::streamable_http_server::tower::{StreamableHttpService, StreamableHttpServerConfig}` + `...::session::local::LocalSessionManager`. `StreamableHttpService::new(service_factory: impl Fn() -> Result<S, io::Error>, session_manager: Arc<M>, config)`. S=TunaSearchServer(Service<RoleServer>). tower 서비스라 axum에 마운트.
- **Cargo**: rmcp에 `transport-streamable-http-server` feature, `serve = ["mcp", "dep:axum", "rmcp?/transport-streamable-http-server"]` 신규 feature(axum 격리). rmcp가 hyper/hyper-util는 자동.
- **mcp.rs**: `start_http_mcp_server(addr, retriever, reader, token: Option<String>)`: StreamableHttpService::new(팩토리=TunaSearchServer 새 인스턴스, LocalSessionManager::default Arc, config default) → axum Router `.nest_service("/mcp", svc)` + bearer 미들웨어(token Some이면 `Authorization: Bearer <token>` 검사, 불일치 401). TcpListener bind + axum::serve.
- **main**: `--serve-mcp <addr>` + `--token <tok>` 분기(serve+sqlite). --db로 retriever+reader 빌드(기존 --mcp-search stdio 분기 답습) 후 HTTP로 서빙.
- **테스트**: 랜덤 포트 기동 + reqwest로 JSON-RPC initialize+tools/call(search_context) 왕복 200, 토큰 불일치 401.
