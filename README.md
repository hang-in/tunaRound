...
같은 SQLite DB(`--db`)를 가리키면 여러 터미널에서 같은 세션을 공유할 수 있습니다.

```bash
tunaround chat --db shared.db --session <id>   # 기존 세션 이어서 진행
tunaround chat --db shared.db --observe <id>   # 읽기 전용 관찰(2초 폴링 tail)
```

이 기능은 한 터미널에서는 토론을 진행하고, 다른 터미널에서는 진행 상황만 볼 때 사용합니다.
(Redis 기반 관찰은 제거되었습니다. 관찰/재개는 이제 SQLite DB를 공유합니다.)
...
> 로컬 관찰(`--observe`)은 이 구조와 별개입니다. 관찰은 같은 SQLite DB를 공유하는 용도이고, 머신 간 공유와 작업 위임은 코어 방식을 사용합니다.

---

## 현재 상태

v1 본체와 v2 검색·맥락 기능이 대부분 들어왔고, A2A 작업 위임과 분산 실행이 확장되었습니다. 0.3.0에서 오케스트레이션 레이어(에이전트 레지스트리·감독 모드·codex 라이브 감독·총괄 웹 대시보드·유니버설 세션 버스)가 들어왔습니다. 대시보드에서 로컬/LAN 세션을 로스터로 관측하고 목표를 위임합니다.
...
- 총괄 웹 대시보드 (`/dashboard`): 로스터 + 라이브 task 피드(카드·펼침) + 목표 제출(loopback)
- presence 스캐너 (v2-44): 머신당 데몬 1개가 라이브 세션(claude·codex)을 로스터에 일괄 동기화 + 세션 수신 자동 가동 훅
- codex-relay (v2-46): 로스터에 보이는 codex 세션 thread로 task를 직접 배달(app-server ws 주입, 대리 claim)
...
- 세션 공유와 관찰 기능의 초기 아이디어는 **tunaSalon**에서 가져왔습니다(현재 구현은 SQLite 기반).
...
- [x] 총괄 웹 대시보드 (roster / 라이브 task 피드(카드) / 목표 제출. codex 직접 제어는 v2-45에서 제거, task 장부 경유 codex-relay가 대체)
- [x] 유니버설 세션 버스 (세션 자동무장 훅. 발견 리포터·후보 패널은 presence 스캐너(v2-44)로 대체)
- [ ] 세션을 넘나드는 프로젝트 기억
- [ ] 리치 TUI
