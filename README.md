...
- 감독 모드: heartbeat 상시 online 로스터, codex 라이브 감독(app-server ws)
- claim 후 워커 사망 시 lease 기반 자동 재큐 + 고착/미배달 표시
- 총괄 웹 대시보드 (`/dashboard`): 로스터 + 라이브 task 피드(카드·펼침) + 목표 제출(loopback) + codex 직접 제어
- 유니버설 세션 버스 (v2-40): SessionStart 자동무장 훅(opt-in) + `tunaround discover`(로컬 세션 발견 리포터) + 대시보드 후보 패널(cross-machine, automation 노이즈 필터)
...
- [x] 총괄 웹 대시보드 (roster / 라이브 task 피드(카드) / 목표 제출 / codex 직접 제어)
- [x] 유니버설 세션 버스 (세션 자동무장 훅 + 발견 리포터 + 대시보드 후보 패널 + codex turn/start 직접 제어)
- [ ] 세션을 넘나드는 프로젝트 기억
- [ ] 리치 TUI
