...
- `--features semantic`: bge-m3 임베딩(원격 Ollama, reqwest) + 벡터 검색 + 하이브리드(BM25 + 의미, RRF 융합). 엔드포인트는 `TUNAROUND_OLLAMA_URL`(기본 `http://127.0.0.1:11435`).
- `--features mcp`: 에이전트가 토론 중 스스로 검색하는 MCP 도구(`search_context`). 러너가 claude·codex에 검색 서버를 물려 자율 호출하게 합니다.
- 예: `cargo run --features "semantic mcp" -- --db tuna.db`
...
Rust + tokio. 영속은 JSON 파일(전사·세션 상태)이고, 검색 인덱스는 SQLite + FTS5 + 벡터(BLOB)입니다. 의미 임베딩은 원격 Ollama(bge-m3), 에이전트 검색 도구는 MCP(rmcp), 멀티세션은 Redis로 둡니다. 검색(SQLite + 형태소)은 기본 빌드에 포함하고, 네트워크/무거운 계층(임베딩·MCP)은 opt-in 피처로 둡니다(가벼운 빌드는 `--no-default-features`). 첫 UI는 thin REPL이고 ratatui·웹 UI는 이후 단계입니다.
